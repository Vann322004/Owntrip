...
exports.HotelController = {
    ...
    getHotelFullPage: async (req, res) => {
        try {
            ...
            const topReviews = await review_model_1.default.aggregate([
                { $match: { targetId: id } },
                { $sort: { createdAt: -1 } },
                { $limit: 10 },
                {
    ...
    postReview: async (req, res) => {
        try {
            ...
            let review = await review_model_1.default.findOne({ userId, targetId });
            if (review) {
    ...
    getMyReview: async (req, res) => {
        try {
            ...
            const review = await review_model_1.default.findOne({ userId, targetId: id });
    ...
    deleteReview: async (req, res) => {
        try {
            const { id } = req.params;
            const userId = req.user?.userId;
            const review = await review_model_1.default.findOneAndDelete({ userId, targetId: id });
            if (!review) {
            ...
            if (stats.length > 0) {
                await hotel_model_1.default.findOneAndUpdate({ hotelId: id }, {
                    "reviewSummary.score": stats[0].avgScore,
                    "reviewSummary.count": stats[0].count,
                    "reviewSummary.cleanliness": stats[0].avgClean,
                    "reviewSummary.service": stats[0].avgService,
